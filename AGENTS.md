...
- **`CabbyCodes.freezeTime.beginAdvance()` — scoped bypass for running sleep-style cascades under freeze.** Returns a token with `.release()`. While any token is held, the var-19 drain, the command117 HourPassed/newDay block, the restore loop, and the safety-net drift check all stand aside. On release (refcount → 0) the freeze snapshot is resynced to current values so freeze resumes from the advanced state. Use this when a cheat explicitly wants to push the clock forward and have TimePasses → HourPassed → newDay run even under freeze (see `cabbycodes-set-time.js` freeze-on forward path). Pair it with a synchronous Game_Interpreter so release happens deterministically after the cascade completes; don't rely on a timer.
...
- **Keep both READMEs in sync.** Any change that adds, removes, or renames a player-facing setting registered via `CabbyCodes.registerSetting` must update the matching bullet in **both** `README.md` (the GitHub markdown copy) and `README.bbcode` (the forum BBCode copy distributed alongside releases). If you change a feature's behavior enough that the existing blurb is misleading, rewrite the blurb in both files in the same commit. Both files split their feature list into two sections — **QoL** and **Cheats** — mirroring the in-game menu; place the new bullet under the same section in each file, in the same order. The bullet order *within* a section, and the set of features in each section, must match between the two files. The blurbs themselves can differ in length (`README.md` tends to be more verbose, `README.bbcode` more terse for forum browsability), but they must describe the same behavior. Support-only modules (`book-ui`, `item-editor`, `oven-navigation`, `version-display`, `time-advance-logger`, and the core/patches/settings/logger/session-state/debug infrastructure) do not get README entries in either file.
