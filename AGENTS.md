...
- **Keep both READMEs in sync.** Any change that adds, removes, or renames a player-facing setting registered via `CabbyCodes.registerSetting` must update the matching bullet in **both** `README.md` (the GitHub markdown copy) and `README.bbcode` (the forum BBCode copy distributed alongside releases). If you change a feature's behavior enough that the existing blurb is misleading, rewrite the blurb in both files in the same commit. The two files have different organization (`README.md` splits by QoL vs Cheats to mirror the in-game menu; `README.bbcode` groups by category for forum browsability) — preserve each file's structure when slotting a new bullet in, but the underlying set of features described must match. Support-only modules (`book-ui`, `item-editor`, `oven-navigation`, `version-display`, `time-advance-logger`, and the core/patches/settings/logger/session-state/debug infrastructure) do not get README entries in either file.
